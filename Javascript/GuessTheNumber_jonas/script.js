...
const refresh = document.getElementById('again');
let x = Math.floor(Math.random() * 20) + 1;
function check() {
    const number = Number(document.querySelector('.number').value);
    if (number === x) {
        text.innerText = "won 😊";   //windows + . 
        document.querySelector('.message').textContent = `correct number 🎉`;
    }
    else {
        text.innerText = "try again ✨";
        if (number > x) {
            document.querySelector('.message').textContent = `the number is greater...!! Guess again `;
        }
        else {
            document.querySelector('.message').textContent = `the number is smaller..!! Guess again `;
        }
...
refresh.addEventListener('click', () => {
    text.innerText = "Guess the number ?";
